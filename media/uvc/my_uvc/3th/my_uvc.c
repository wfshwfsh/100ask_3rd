...
static void parse_videocontrol_interface(const unsigned char *buf, int buflen)
{
...
static void parse_videostreaming_interface(unsigned char *buf, int buflen)
{
...
void dump_uvc_intfAlt_desc(struct usb_interface_descriptor *intf)
{
    ...
    printk("      bLength:            %5u\n", intf->bLength);
    printk("      bDescriptorType:    %5u\n", intf->bDescriptorType);
    printk("      bInterfaceNumber:   %5u\n", intf->bInterfaceNumber);
    printk("      bAlternateSetting:  %5u\n", intf->bAlternateSetting);
    printk("      bNumEndpoints:      %5u\n", intf->bNumEndpoints);
    printk("      bInterfaceClass:    %5u\n", intf->bInterfaceClass);
    printk("      bInterfaceSubClass: %5u\n", intf->bInterfaceSubClass);
    printk("      bInterfaceProtocol: %5u\n", intf->bInterfaceProtocol);
    printk("      iInterface:         %5u\n", intf->iInterface);
...
void dump_uvc_conf_desc(struct usb_host_config *config)
{
    ...
    printk("    wTotalLength:         %5u\n", config_desc->wTotalLength);
    printk("    bNumInterfaces:       %5u\n", config_desc->bNumInterfaces);
    printk("    bConfigurationValue:  %5u\n", config_desc->bConfigurationValue);
    printk("    iConfiguration:       %5u\n", config_desc->iConfiguration);
    printk("    bmAttributes:         %5u\n", config_desc->bmAttributes);
...
int myUvc_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	...
	for(i=0;i<descriptor->bNumConfigurations;i++){
        ...
        for(j=0;j<intf->num_altsetting;j++){
            interface = &intf->altsetting[j];
            dump_uvc_intfAlt_desc(&interface->desc);
            
            for(m=0;m<interface->desc.bNumEndpoints;m++){
                endPoint = &interface->endpoint[m];
                dump_endpoint(&endPoint->desc);
            }
        }
        ...
        if ((buffer[1] == USB_DT_CS_INTERFACE) && (interface->desc.bInterfaceSubClass == 1))
        {
            parse_videocontrol_interface(buffer, buflen);
        }
        if ((buffer[1] == USB_DT_CS_INTERFACE) && (interface->desc.bInterfaceSubClass == 2))
        {
            parse_videostreaming_interface(buffer, buflen);
        }
